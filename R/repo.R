...
repo_packages_path <- function(dir = ".", type = c("source", "win.binary", "mac.binary"), r_version = getRversion()) {
  if (rlang::arg_match(type) != "source") {
...
#'   the index for.
#' @export
repo_update <- function(repo, type = c("source", "win.binary", "mac.binary"), r_version = getRversion()) {
  dir <- repo_packages_path(repo, type, r_version)
